...
An injection-recovery study of microlensing completeness and false-positive
rate for the Nancy Grace Roman Space Telescope Galactic Bulge Time Domain
Survey (GBTDS): how do cadence gaps, blending, finite-source effects,
parallax, correlated noise, and detection thresholds change what a
bound-planet or free-floating-planet (FFP) microlensing search actually
recovers?
...
## Result (free-floating-planet channel)

![Completeness surface](results/figures/completeness_default.png)

A 560-trial synthetic injection-recovery grid (`configs/default.yaml`)
recovers **58.0%** of injected free-floating-planet events overall, falling
sharply with fainter source magnitude and larger impact parameter; a
matched 200-trial null test with no injected signal yields **0 false
positives** (95% CI upper bound 1.9%). See `paper/manuscript.md` Section 4
for the full breakdown and what this result does and does not show (it is a
methods-validation result, not a population-level Roman yield forecast).
Every number above is read from `results/default/`, generated by
`romanmlr run-grid` / `romanmlr null-fpr` -- see `docs/REPRODUCIBILITY.md`.
