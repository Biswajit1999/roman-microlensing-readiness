...
_FAST_CADENCE = CadenceConfig(
    season_start_days=(0.0, 15.0), season_length_days=10,
    cadence_minutes=30, random_dropout_frac=0.0,
)


def test_bright_high_snr_ffp_event_is_recovered():
    cfg = TrialConfig(
        channel="ffp", u0=0.05, tE=1.5, rho=0.01, t0=5.0,
...
def test_faint_shallow_event_is_not_recovered():
    cfg = TrialConfig(
        channel="ffp", u0=1.5, tE=0.3, rho=0.01, t0=5.0,
        mag_ref=24.5, seed=0, cadence=_FAST_CADENCE,
    )
    result = run_trial(cfg)
    assert not result.event_detected
...
def test_planetary_channel_is_disabled():
    cfg = TrialConfig(
        ...
        channel="planetary", u0=0.05, tE=20.0, rho=0.02, q=0.01, s=1.0, t0=5.0,
        mag_ref=18.0, seed=0, cadence=_FAST_CADENCE, grid_n=400,
    )
    with pytest.raises(RuntimeError, match="planetary inference is disabled"):
        run_trial(cfg)


def test_run_grid_returns_one_row_per_config():
    configs = [
        TrialConfig(channel="ffp", u0=u0, tE=2.0, rho=0.01, t0=5.0, mag_ref=19.0,
